...
type TodoPresetModule = { readonly createTodoTopology: (plan: unknown) => Effect.Effect<Topology, unknown> }
...
it.effect("R04 and S06 evolve a four-root Todo baseline and replay the same addition with zero diff", () =>
  Effect.gen(function* () {
    const { ApplyPlan, Evolution, Planner, TodoPreset } = yield* modules()
    const tree = createTreeWithEmptyWorkspace()
    const fullTopology = yield* TodoPreset.createTodoTopology(
    ...
    expect(tree.exists("packages/todo/domain/package.json")).toBe(true)
    expect(tree.exists("packages/todo/application/package.json")).toBe(true)
    expect(tree.exists("packages/todo/infrastructure/package.json")).toBe(true)
    expect(tree.exists("apps/todo-cli/package.json")).toBe(true)
