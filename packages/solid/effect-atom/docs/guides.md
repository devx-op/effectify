...
export const completedTodosAtom = Atom.make((get) => get(todosAtom).filter((todo) => todo.completed))
...
export const userAtom = Atom.fn((get) => fetchUser(get(userIdAtom)))
...
export const addTodoAtom = Atom.fn((get, title: string) =>
  Effect.gen(function*() {
    ...
    yield* Effect.sync(() => optimisticTodosAtom.set([...currentTodos, tempTodo]))
    ...
    try {
      ...
      const updatedTodos = get(optimisticTodosAtom).map((todo) => todo.id === tempId ? newTodo : todo)
      yield* Effect.sync(() => optimisticTodosAtom.set(updatedTodos))
      ...
      yield* Effect.sync(() => optimisticTodosAtom.set(rolledBackTodos))
...
export const retryableDataAtom = Atom.fn(() =>
  Effect.gen(function*() {
    const data = yield* Effect.tryPromise(() => fetch("/api/unreliable-endpoint").then((r) => r.json()))
    return data
  }).pipe(
    Effect.retry({
      times: 3,
...
// Mock the API
vi.mock("./api", () => ({
  fetchData: vi.fn(() => Effect.succeed({ message: "Test data" })),
}))
