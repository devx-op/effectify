...
export const makeUseEffectQuery = <R>(createRunner: Runner<R>) =>
<
  TData,
...
> => {
  ...
  const queryFn: QueryFunction<TData, TQueryKey> = createMemo(
    () => (context: QueryFunctionContext<TQueryKey>) => {
      const effect = (
        options.queryFn as EffectfulQueryFunction<TData, TError, TQueryKey, R>
      )(context)
  ...
  return useQuery(() => ({
    ...options,
    queryKey: options.queryKey as TQueryKey & {},
