      ...
      const output = yield* (
        task._tag === "Durable"
          ? task.execute(decoded, {
              ...taskContext,
              invocationCount: "invocationCount" in taskContext ? taskContext.invocationCount : 0,
            })
          : task.execute(decoded, taskContext)
      ).pipe(Effect.provideService(Scope.Scope, taskScope), Effect.provideContext(context))
...
export const make = (): Registry => {
  ...
  return {
    ...
    run: <Output, Error>(name: string, input: unknown, taskContext: Task.Context | Task.DurableContext) => {
