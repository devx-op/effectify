...
const WorkerLabels = Schema.Record(
  Schema.String,
...
export const fromEnv: Config.Config<HatchetOptions> = Config.all({
  ...
  namespace: optional(
    Config.schema(Schema.NonEmptyString, "HATCHET_NAMESPACE"),
  ),
  ...
  workerName: Config.schema(Schema.NonEmptyString, "HATCHET_WORKER_NAME").pipe(
    Config.withDefault("hatchet-worker"),
  ),
  workerSlots: optional(Config.schema(PositiveInteger, "HATCHET_WORKER_SLOTS")),
  readyTimeoutMs: optional(
    Config.schema(PositiveInteger, "HATCHET_WORKER_READY_TIMEOUT_MS"),
  ),
  stopTimeoutMs: Config.schema(
    PositiveInteger,
    "HATCHET_WORKER_STOP_TIMEOUT_MS",
  ).pipe(Config.withDefault(5_000)),
}).pipe(
  Config.map((value) => ({
    client: {
      token: value.token,
      ...(Option.isSome(value.hostPort)
        ? { hostPort: value.hostPort.value }
        : {}),
      ...(Option.isSome(value.apiUrl) ? { apiUrl: value.apiUrl.value } : {}),
      ...(Option.isSome(value.tenantId)
        ? { tenantId: value.tenantId.value }
        : {}),
      ...(Option.isSome(value.namespace)
        ? { namespace: value.namespace.value }
        : {}),
      ...(Option.isSome(value.logLevel)
        ? { logLevel: value.logLevel.value }
        : {}),
      ...(Option.isSome(value.tlsStrategy)
        ? { tlsStrategy: value.tlsStrategy.value }
        : {}),
    },
    worker: {
      name: value.workerName,
      ...(Option.isSome(value.workerSlots)
        ? { slots: value.workerSlots.value }
        : {}),
      ...(Option.isSome(value.readyTimeoutMs)
        ? { readyTimeoutMs: value.readyTimeoutMs.value }
        : {}),
