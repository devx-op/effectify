...
type LiveDeclaration = ReturnType<
...
const makeLiveService = (
	...
	Effect.gen(function* () {
		...
		let lifecycle: "collecting" | "starting" | "ready" | "closing" =
			"collecting";
								...
								Effect.gen(function* () {
									...
									const stopped = yield* Effect.tryPromise({
										try: () => worker.stop(),
											...
											new WorkerStopDefect(originalCause),
									}).pipe(
		...
		Effect.gen(function* () {
			...
			const sdkClient = yield* Effect.try({
				try: () => {
					...
					new HatchetConfigError({ field: "client", originalCause }),
			});
