...
export class HttpResponseFailure<T = unknown> extends Data.TaggedClass(
  "HttpResponseFailure",
)<{
  readonly cause: T
}> {}
...
export const httpSuccess = <T>(
  data: T,
): Effect.Effect<HttpResponseSuccess<T>, never, never> => Effect.succeed(new HttpResponseSuccess({ data }))

export const httpFailure = <T = unknown>(
  cause: T,
): Effect.Effect<HttpResponseFailure<T>, never, never> => Effect.succeed(new HttpResponseFailure({ cause }))
