...
export const make = <R, E>(layer: Layer.Layer<R, E, never>) => {
  ...
  const withLoaderEffect =
    <A, B>(self: Effect.Effect<HttpResponse<A> | Response, B, R | LoaderArgsContext>) => (args: LoaderFunctionArgs) => {
      const runnable = pipe(
        self,
  ...
  const withActionEffect =
    <A, B>(self: Effect.Effect<HttpResponse<A> | Response, B, R | ActionArgsContext>) => (args: ActionFunctionArgs) => {
      const runnable = pipe(
        self,
        Effect.provide(Logger.pretty),
        ...
        Exit.match({
          onFailure: (cause) => {
            if (cause._tag === "Fail") {
              ...
              return json({ ok: false as const, errors: [String(error)] }, { status: 400 })
            }
