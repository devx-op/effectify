...
app.post('/api/register', (req, res) => {
  ...
  Effect.runPromise(
    AuthService.signUp(email, password, name).pipe(
      Effect.map(result => res.status(201).json({
        success: true,
        user: result.user,
        session: result.session
      })),
      Effect.catchTag('AuthenticationError', error =>
        Effect.sync(() => res.status(400).json({
          error: error.message
        }))
      ),
...
app.post('/api/login', (req, res) => {
  ...
  Effect.runPromise(
    AuthService.signIn(email, password).pipe(
      Effect.map(result => res.json({
        success: true,
        user: result.user,
        session: result.session
      })),
      Effect.catchTag('AuthenticationError', error =>
        Effect.sync(() => res.status(401).json({
  ...
  Effect.runPromise(
    authenticateRequest.pipe(
      ...
      Effect.catchTag('InvalidTokenError', () =>
        Effect.sync(() => res.status(401).json({ error: 'Invalid token' }))
      ),
    ...
    Effect.runPromise(
      checkRole.pipe(
        ...
        Effect.catchTag('AuthorizationError', error =>
          Effect.sync(() => res.status(403).json({ error: error.message }))
        )
      )
    )
  }
...
export class UserService extends Context.Tag("UserService")<
  UserService,
  {
    ...
    readonly changePassword: (id: string, oldPassword: string, newPassword: string) => Effect.Effect<void, AuthenticationError | ValidationError>
  }
>() {}
