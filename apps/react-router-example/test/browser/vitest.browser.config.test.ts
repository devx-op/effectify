...
import { defaultBrowserTestBaseUrl } from "./browser-test-config.js"
...
describe("vitest browser config", () => {
  it("isolates browser tests and points to the reusable base url", () => {
    ...
    expect(testConfig.include).toEqual(["test/browser/**/*.browser.test.ts"])
  ...
  it("includes browser test sources in tsconfig.test.json", () => {
    ...
    expect(tsconfig.include).toContain("./test/browser/**/*.ts")
  })
})
