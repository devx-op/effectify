...
const withConfig = (values: Record<string, string>) =>
  authOptionsConfig.pipe(Effect.provide(ConfigProvider.layer(ConfigProvider.fromUnknown(values))))
...
describe("Better Auth Effect configuration", () => {
  ...
  it("rejects a whitespace-only secret without rendering it", async () => {
    ...
    const error = await Effect.runPromise(withConfig({ BETTER_AUTH_SECRET: suppliedSecret }).pipe(Effect.flip))
  ...
  it("passes a valid secret only to the Better Auth options boundary and preserves the URL", async () => {
    ...
    const options = await Effect.runPromise(withConfig({ BETTER_AUTH_SECRET: suppliedSecret }))

    expect(options.secret).toBe(suppliedSecret)
    expect(options.baseURL).toBe(process.env.BETTER_AUTH_URL ?? "http://localhost:4200")
    expect(JSON.stringify({ baseURL: options.baseURL })).not.toContain(suppliedSecret)
  })
})
...
describe("Hatchet run README contract", () => {
  it("documents the required local Better Auth secret setup", async () => {
    ...
    expect(readme).toContain('export BETTER_AUTH_SECRET="$(openssl rand -base64 32)"')
  ...
  it("documents the current authenticated asynchronous acceptance route", async () => {
    ...
    expect(readme).toContain("cookie: better-auth.session_token=<session-cookie>")
