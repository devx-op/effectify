...
function generateModelOperations(models: DMMF.Model[], customError: CustomErrorConfig) {
  return models
    .map((model) => {
      ...
      const promiseCast = (op: string, nullable = false) => {
      ...
      const resultType = (op: string, nullable = false) => {
      ...
      const errorType = (opErrorType: string) => (customError ? customError.className : opErrorType)
      const mapperFn = (defaultMapper: string) => (customError ? 'mapError' : defaultMapper)
