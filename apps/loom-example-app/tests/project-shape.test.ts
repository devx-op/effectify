...
describe("loom example app project shape", () => {
  ...
  it("exports behavior-first route contracts through public entry points without extra bootstrap files", async () => {
    ...
    expect(documentResult.html).toContain('src="/src/entry-client.ts"')
    expect(existsSync(new URL("../src/entry-browser.ts", import.meta.url))).toBe(false)
    expect(existsSync(new URL("../src/jsdom.d.ts", import.meta.url))).toBe(false)
    expect(existsSync(new URL("../src/router-runtime.ts", import.meta.url))).toBe(false)
    expect(existsSync(new URL("../src/routes/todo-route-submission.ts", import.meta.url))).toBe(false)
