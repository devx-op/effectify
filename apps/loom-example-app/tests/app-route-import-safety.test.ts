...
describe("loom example app import-time SSR safety", () => {
  ...
  it("imports template-authored routes and router without installing a global document", async () => {
    const counterRouteModule = await importFresh<{ component: unknown }>("../src/routes/counter-route.ts")
    const todoRouteModule = await importFresh<{ component: unknown }>("../src/routes/todo-route.ts")
    const routerModule = await importFresh<{ appRouter: unknown }>("../src/router.ts")

    expect(counterRouteModule.component).toBeDefined()
    expect(todoRouteModule.component).toBeDefined()
