...
| Split WU6 into PR6a runtime/source and PR6b dependency prune                                   | Count deterministic lockfile prune in PR6a; keep activation/removal atomic  | PR6a is green with unused declarations and no lockfile change; PR6b carries the authorized ~2,727 generated-line exception with tiny authored changes. |
...
**WU6 / PR6a — RR7 runtime/source activation (forecast 350–500 authored, 0 generated).** In one green transaction, switch scripts to `react-router dev`/`react-router build`/`react-router-serve`, switch Vite to `reactRouter()`, replace UI/route APIs with `react-router`, stream conversion with `@react-router/node`, hydration with `HydratedRouter` from `react-router/dom`, and SSR with `ServerRouter` plus RR7 `EntryContext`. The same transaction migrates entries and remaining source, preserves the application-owned RR7 Better Auth adapter, and proves zero active Remix imports, browser/server components, commands, or Vite plugin references. The four exact legacy `@remix-run/{node,react,serve,dev}` dependencies remain temporarily declared but unused, and `pnpm-lock.yaml` is unchanged, so PR6a is green but does not claim dependency-residue removal or complete the no-Remix dependency checkpoint.

**WU6 / PR6b — legacy dependency and generated-lockfile prune (forecast 20–60 authored, ~2,727 generated).** Remove only the four unused legacy declarations and regenerate with `pnpm install --lockfile-only`. The maintainer explicitly authorized the deterministic ~2,727 generated-line exception after attempt 1 measured the prune; authored changes stay tiny. PR6b contains no runtime/source behavior change and completes the dependency-residue requirement. A state with RR7 execution and Remix-shaped source remains forbidden; PR6a avoids that state, while declared-but-unused dependency residue is an explicitly bounded intermediate graph state.
...
| `apps/react-remix-example/package.json`, project/Nx/TS/Vite configs        | WU5 stages RR7; PR6a activates commands/plugin while keeping unused legacy deps; PR6b removes them            | Delete app/importer.                                                        |
...
| `pnpm-lock.yaml`                                                           | PR6a leaves WU5 identity unchanged; PR6b deterministically prunes unused Remix graph under approved exception | Regenerate to remove all RR7/Remix importers/resolutions.                   |
...
| RR7 app               | WU5 declarations/routes, clean typegen→typecheck, Remix build/tests; WU6 PR6a RR7 runtime/source activation with declared residue, then PR6b dependency/lockfile prune; RR7 SSR/runtime/build. | App and importer absent after ledger completion.                                                                                                                 |
...
6. **RR7 activation split:** PR6a switches scripts/plugin and migrates entries/route/UI imports, `HydratedRouter`, `ServerRouter`, node stream types, and SSR/source-residue tests while retaining unused legacy declarations and the WU5 lockfile (forecast 350–500 authored, 0 generated). PR6b removes only those declarations and deterministically prunes ~2,727 generated lockfile lines under the explicitly approved exception (forecast 20–60 authored).
...
- During PR6a, roll back scripts/plugin/entries/source/tests to WU5 while leaving manifests and lockfile untouched. During PR6b, restore the four legacy declarations and exact PR6a lockfile without reverting the green RR7 runtime/source. Later RR7 importer rollback still restores all four exact `7.18.2` pins together.
...
| RR7 execution activates against Remix-shaped source             | WU5 retains Remix execution; PR6a switches runtime and source together. PR6b changes only unused declarations/lockfile. |
| Generated dependency prune exceeds ordinary line budget         | PR6a leaves lockfile unchanged; authorized PR6b reports ~2,727 generated lines separately with tiny authored scope.     |
