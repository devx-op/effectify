...
if(cmd==='git'){
 ...
 console.log(JSON.stringify(a[2]==='versions'?v.versions:v.latest));save()
}else if(cmd==='pnpm'){
 let names=a[3].replace('--projects=','').split(',');for(const n of names){let rec=s.expected[n];s.npm[n]={...(s.npm[n]||{}),versions:[rec],latest:rec}}save()
